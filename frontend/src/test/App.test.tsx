...
describe("App", () => {
  ...
  it("shows top menu navigation and tenant-style novel management after login", async () => {
    ...
    expect(screen.getByRole("menuitem", { name: "评分" })).toBeInTheDocument();
  ...
  it("updates the browser path when switching workspace menu sections", async () => {
    ...
    await user.click(screen.getByRole("menuitem", { name: "评分" }));
    expect(window.location.pathname).toBe("/scoring");
